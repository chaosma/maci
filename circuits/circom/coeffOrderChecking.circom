...
template CoeffOrderChecking(stateTreeDepth, coeffTreeDepth) {
    var stateTreeArity = 5;
    var coeffTreeArity = 5;
    ...
    var TREE_ARITY = 5;
    component ballotTree = QuinCheckRoot(stateTreeDepth);
    for (var i = 0; i < TREE_ARITY ** stateTreeDepth; i++) {
        ballotTree.leaves[i] <== ballotHashs[i];
    }
    ballotTree.root === ballotTreeRoot;

    component ballotTree1 = QuinCheckRoot(coeffTreeDepth);
    for (var i = 0; i < TREE_ARITY ** coeffTreeDepth; i++) {
        ballotTree1.leaves[i] <== ballotHashs1[i];
    }
    ballotTree1.root === ballotTreeRoot1;

    component ballotTree2 = QuinCheckRoot(coeffTreeDepth);
    for (var i = 0; i < TREE_ARITY ** coeffTreeDepth; i++) {
        ballotTree2.leaves[i] <== ballotHashs2[i];
    }
